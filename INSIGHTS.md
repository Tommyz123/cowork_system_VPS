...
[2026-07-01] [P9/测试方法] 测触发器/守卫必须用真实存在的行 → 用不存在的id测UPDATE/DELETE会零行命中显示"成功"=假阴性(触发器按行触发,零行=不触发)。今天二审实锤:用id=1(表从id=2起)测冻结触发器差点误报"没拦住",换真实行id=2实测被拦。通用于一切守卫类测试:动手前先SELECT确认目标行存在。
