...
**第1层 P9 = AI 自动实验田**
- swing 账号、纸钱（equity/cash 一律实时查 Alpaca，文档不写死数字；旧记 $1M 是错的，已废弃）、全自动
- 任务 = 验证"AI 自动选股行不行"；**12 月验收定去留，期间不动不加码**
- 这一层就是下面的 TIDE 系统

**第2层 趋势主线 = 主力方向**
- intraday 账号（余额实时查 Alpaca，旧记 $1M 是历史误记勿引用；至今 0 交易未接代码；真试水建议重置 $20-50k）
...
## 当前阶段原则（第1层）
学习阶段（纸账号 swing；equity/cash 实时查 Alpaca，不写死），积累 signal 数据是第一优先级。
