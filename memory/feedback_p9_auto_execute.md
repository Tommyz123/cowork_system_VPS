---
name: P9 研究阶段自动下单规则
description: 研究阶段（paper trading）P9 cognitive_scanner 扫描后直接下单 swing，4 层 sanity check 替代人工 approve gate；每只 $1000/季度上限 top25（2026-05-30 攒样本提速）；转真钱时必须重新设计
type: feedback
...
1. **cognitive_scanner.py 行为**（2026-05-19 修正——反模式根治）：
   - 扫到 top picks（评分 ≥ 5）→ 立即调 alpaca swing place_order opg 单（次日开盘成交）
   - DB INSERT `status='submitted', cohort='auto_pending', signal_date=scan_date, signal_entry_price=current_price`（**不再硬编码 'filled'**——OPG 单实测 1/6 fill 率，不能假设 100% 成交）
   - trades 表同步写入（status='open', fill_price=NULL）
   - 次日 9:45 EDT sync_fill_prices.py（reconciler）按 Alpaca 实际 status 更新：
     - filled → scanner_picks.status='filled' / cohort='auto_pending'→'auto_filled' + 回填 fill_entry_price/fill_date/spy_fill_entry
     - expired/canceled/rejected → DB 同步成同名状态
   - RCA: `trading/rca/2026_05_19_opg_expired_anti_pattern_recurrence.md`
...
2. **4 层 sanity check（顺序执行，任一拒绝就跳过该只）**（2026-05-18 A1 方案升级）：
   ...
   - **(b) 单只 notional 上限 $2000**（2026-05-30 由 $5000 降）：qty=round(1000/price)；qty*price > 2000 → 拒绝
   - **(c) 单次扫描下单数量上限 30 只**（2026-05-30 由 15 升，配合 top25）：防 scanner bug 量级失控
   - **(d) buying_power 充足检查**（A1 新增）：扫描开始拉 swing buying_power，每只下单前检查剩余 buying_power >= $1000 (NOTIONAL_TARGET)，不足则拒所有后续 + Discord 警报。防止 paper account 耗尽时 Alpaca 403 拒单产生 ghost positions。
...
6. **每只仓位标准化**：历史 5/06 + 5/11 + 5/18 三个 cohort 为 $3000；**2026-05-30 起新仓降为 $1000**（纸账号只需样本数据，hit rate 算 return% 与金额无关；缩小金额→同现金埋更多票→更快攒样本）。⚠️ attribution 横向比要按 return% 不按绝对金额（金额已不再统一）。同时季度埋伏上限 top10→top25（季度节奏不变，框架内拓宽广度 + 引入分数梯度验证评分系统）。
