...
**最后 Push 到 GitHub：**
> ⚠️ 收工例外：主公说「收工」时 `discord_approve.py` 已自动写 `/tmp/git_approved_<实例>`（内容 savework），git 守卫见此标记**放行且不消耗**，覆盖整个收工的 commit + push，无需手动 touch。收工结束由 UserPromptSubmit 的 rm 逻辑清理。
```bash
