...
- [2026-06-27, 做完unfilled_tracker后主公提] **新闻追踪自动持续化** | 背景：今天做完"过期票股价追踪+对答案"(unfilled_tracker)闭环后，主公问"追踪新闻时也会自己继续追踪吗"——我澄清那套追的是股价非新闻；主公让把"新闻追踪"单列为话题。现状=P9新闻侧已有 catalyst_monitor/signal_collector/FERC哨兵 各自在跑(采信号/盯催化剂/盯监管)，但各为政、缺"对某主题/某事件持续追踪、累积成档案"的闭环(类似股价对答案那样的"押了新闻→后续验证")｜待讨论点：①主公到底想要什么(某主题新闻持续聚合？哨兵报告自带解读？信号→后续兑现验证？)②与已有"哨兵报告自动解读B方案"(loop设计§6.5,缓做)是否同一需求 ｜触发：下次主公明确新闻追踪具体诉求时，先对齐到底要哪种，再判与现有脚本/loop设计的关系，避免重复造
...
- [2026-06-24, P9审核连环误报后挂起] **关键文档「同一事实两个值」自动扫描** | 背景:审核连报3个假问题,根因=文档里旧值没清(账号$1M旧说法/手抄会过期数字),靠自觉读对文档治不了根 | 设想:收工流程加轻量扫描——扫关键事实文档(playbook/memory)有没有同一事实出现两个不同数值,有就提醒就地覆盖 | 缓做原因:坑频率低(2026-06-24 当天2个+6/23 1个),现在搭自动校验=过度工程(YAGNI),维护脚本本身又成新「追不上」 | 触发:「新旧并存」坑再犯第3次,ROI才够
  → [关联评估 2026-06-26]：主公提"实例配置↔文档自动同步(改了配置必触发回写)"，CC评估同族结论——核心约束=手动改service不经Claude故hook无效,唯一可靠=定时校验;grep friction证配置漂移痛点一年个位数,常驻监控违YAGNI+token经济;**主公拍板"今天先别做"**。保险方案=并入本条触发条件，下次收工时加轻量"配置↔文档一致性快检"(读运行时真相比对reference_dual_bot,零常驻),或漂移漏掉反复出现再升级
