...
## [草稿] 2026-06-29 深度审核（session a28b2f70）

> 本次为 Loop Engineering 讲解 + P9 提速攒样本讨论（中途跑偏第1层1小时，主公澄清本意第2层）。
> 核心成果已落档 trading/notes/第1层实验设计规格_草案_20260629.md，不重复送审。

### 🤖 本次自动写入摘要（4-5 分，已直接写入正式文件）
- **[评分:4]** Friction | 多层/多模块项目接指令若词有歧义→动手前先确认指向哪层(本次P9第1/2层跑偏1小时实锤；根因=违反understand_before_act+P9双层结构天然歧义) → 已写入 friction_log.md【待验证】

### 说明（无 2-3 分送审项）
- 本次第1层洞察(成交率≠排序力主统计/瓶颈是评分通胀非频率)均为第1层specific，已完整落档草案文档，不重复进送审列表。

## [草稿] 2026-06-30 深度审核

### 操作记录 建议起草（1份）
- **[评分:3]** 主题：Sage Seeds 引流数据源能力地图（Weedmaps/Google商家API可行性）/ 背景：本次查证确认 Google商家有官方 Business Profile Performance API 可拉曝光/点击/搜索词(需老板OAuth+填基础访问申请表,新项目初始配额0)；Weedmaps官方API只有Menu/Orders无分析数据 / 建议文件名：放进 sage_seeds 项目内或 playbooks/career_ops.md「引流数据源」区块 / 价值：引流线推进时直接查，省得重新摸索；但API政策会变，写时标日期+"实时查官方"

### 🤖 本次自动写入摘要（4-5 分，已直接写入正式文件）
- **[评分:5]** 数据分析方法论「数据聚合前必做常识校验+口径核验」(花$9错误升华为通用规则) → 已写入 reference/knowledge_base.md

## [草稿] 2026-07-01 深度审核（3 sessions：2cadd4ad本场P9提速实验 / a9f440d2 BB升Fable5 / 83118c69 重启确认+bot改名）

### INSIGHTS 建议写入（1条）
1. **[评分:3]** 「新实验/方案设计前，先扫同类历史审核存档当 checklist」→ 本场实锤：第2层方案 v0.1 被 Fable5 审核逮住**两条重犯第1层 Codex 已买教训**（±N%三分类丢排序信息 / 关键判断值无可操作定义）——教训明明躺在 trading/notes/codex_*review*.md 里，写 v0.1 时没当 checklist 过。规则雏形：写实验规格/系统方案前，grep 同项目（或同类）的 *audit* / *review* 存档，把历史致命项逐条对照过一遍再动笔；已买的教训不复用=白买。[src:2cadd4ad]

### 其他类别
- 本场核心产出（提速实验方案v0.2/三本账/七道防污染闸/审核修法）已全部落正式文档（方案+audit存档+playbook+作战表），不重复送审。
- "越说越乱"教训已经主公批准写入 memory/feedback_discord_details_to_docs.md（闭环）。
- 收工SKILL corrections统计bug已实时记 friction_log（待讨论）。
- a9f440d2（BB模型opus-4.8→fable-5升级+重启）/ 83118c69（重启确认+BB bot名改BB-Fable5）：操作已记日志，无新规则，价值1分丢弃；牵出的权威表滞后已修（见下方自动写入）。

---

### 🤖 本次自动写入摘要（4-5 分，已直接写入正式文件）
- **[评分:4]** 文档对齐 reference_dual_bot.md 三处滞后修正 → BB模型 opus-4-8→**fable-5**（7/1升级）/ 昵称行 AA-Sonnet5+BB-Fable5 / "BB/CC对fable-5无权限"警告改写为"权限随订阅会变，改模型后必须实测能回消息，CC未验证改前先测"（旧断言已被7/1 BB实测证伪，按 doc_single_source 就地改干净）

### 🗑️ 本次自动丢弃摘要（1 分，未保留）
- 共 2 条 1 分候选丢弃（83118c69 重启确认+按既有规则改bot名 / a9f440d2 模型升级操作记录，均已记日志无新洞察）
