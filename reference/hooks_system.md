...
> Hook 文件位置：`~/.claude/hooks/` 和 `cowork/scripts/`
...
| UserPromptSubmit | `rm -f /tmp/task_approved`（内联） | 收到消息 | 清除上轮授权 token，防止跨任务串用 |
...
#### `git_commit_guard.sh`
- **触发**：任何 Bash 命令
- **逻辑1（git 守卫）**：命令含 `git commit` 或 `git push` → 拦截，提示先列变更等主公确认；需主公执行 `touch /tmp/git_approved` 解锁
- **逻辑2（touch 守卫）**：命令含 `touch /tmp/task_approved` → block，提示"Claude 无权自行授权，需主公通过 Discord 触发"
- **为什么**：git 操作不可逆；task_approved 自授权等于无守卫

#### `system_file_guard.sh`
- **触发**：Edit 或 Write 工具
- **白名单**（直接放行）：cowork_log.md、CURRENT_SESSION.md、friction_log.md、INSIGHTS.md、auto_pending.md、archive/、/tmp/
- **非白名单**：检查 `/tmp/task_approved` 是否存在，不存在则拦截，提示走授权流程
- **为什么**：防止 Claude 自作主张修改系统文件
...
- **逻辑**：检测 prompt 里是否来自 Discord（含 `source="plugin:discord:discord"`）且包含关键词 → `touch /tmp/task_approved`，注入授权确认消息
...
主公发消息 → UserPromptSubmit
  ├─ rm task_approved（清除上轮授权）
  ├─ discord_approve.py（检测授权关键词）
  │    └─ 含关键词 → touch /tmp/task_approved
  └─ 其他提醒 Hook

Claude 准备修改文件 → PreToolUse
  └─ system_file_guard.sh
       ├─ 白名单 → 放行
       └─ 非白名单 → 检查 task_approved
            ├─ 存在 → 放行
            └─ 不存在 → 拦截，提示走授权流程

Claude 完成任务 → rm -f /tmp/task_approved
```
